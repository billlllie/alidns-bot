...
async function lookupRRId(domain,keyword) {
    ...
    let out = 'Index\tRecord\tType\tLine(ISP)\tValue\tStatus\t\n';
    let RRid = [];
    record.DomainRecords.Record.forEach((elem,index)=> {
        out += index+'\t';
        out += elem.RR+'\t';
        out += elem.Type+'\t';
        out += elem.Line+'\t';
        out += elem.Value+'\t';
        out += elem.Status=='ENABLE'?'√':'×';
        out += '\n';
    ...
    await client.AddDomainRecord({
        ...
        if (err){
            if (err.data.Code != 'DomainRecordDuplicate') {
                throw Promise.reject(err.data.Message);
            } else if(debug) {
